...
struct view_id_btree_reduction_t {
    uint64_t kv_count;
    bitmap_t partitions_bitmap;
};
...
couchstore_error_t encode_view_btree_reduction(
        const view_btree_reduction_t* reduction,
        char* buffer,
        size_t* buffer_size);

void free_view_btree_reduction(view_btree_reduction_t* reduction);

couchstore_error_t decode_view_id_btree_reduction(
        const char* bytes, view_id_btree_reduction_t** reduction);

couchstore_error_t encode_view_id_btree_reduction(
        const view_id_btree_reduction_t* reduction,
        char* buffer,
        size_t* buffer_size);
