...
struct view_spatial_builder_ctx_t {
...
int spatial_key_cmp(const sized_buf* key1,
                    const sized_buf* key2,
                    const void* user_ctx);

/* Compare keys of a spatial index for the file merger */
int spatial_merger_key_cmp(const sized_buf* key1,
                           const sized_buf* key2,
                           const void* user_ctx);
...
scale_factor_t* spatial_scale_factor(const double* mbb,
                                     uint16_t dim,
                                     uint32_t max);
...
int view_spatial_filter(const sized_buf* k,
                        const sized_buf* v,
                        const bitmap_t* bm);
