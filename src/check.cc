...
static std::string get_couchstore_last_internal_error(Db* db) {
    std::array<char, 256> msg;
    auto status = couchstore_last_internal_error(db, msg.data(), msg.size());
    if (status == COUCHSTORE_SUCCESS) {
        return msg.data();
    }
    return "Failed to get last internal error:" +
           std::string(couchstore_strerror(status));
}
...
static void print_error(Db* db, couchstore_error_t err, std::string_view msg) {
    std::cerr << filename << ": " << std::string(msg)
              ...
              << " last_error:" << get_couchstore_last_internal_error(db)
              << std::endl;
}
